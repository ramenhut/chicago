...
#define EVX_ENTROPY_PRECISION					(32)
#define EVX_ENTROPY_PRECISION_MAX				((uint32)((uint64(0x1) << EVX_ENTROPY_PRECISION) - 1))
#define EVX_ENTROPY_PRECISION_MASK				((uint32)((uint64(0x1) << EVX_ENTROPY_PRECISION) - 1))
...
void entropy_coder::resolve_model_ctx(entropy_context *ctx)
{
    uint64 range = high - low;
    uint64 mid_range = range * ctx->history[0] / (ctx->history[0] + ctx->history[1]);
    mid = low + mid_range;
}
...
evx_status entropy_coder::encode_symbol(uint8 value) 
{
    ...
    if (history[0] + history[1] >= 65536) {
        history[0] = (history[0] >> 1) | 1;
        history[1] = (history[1] >> 1) | 1;
    }

    return EVX_SUCCESS;
}

evx_status entropy_coder::encode_symbol_ctx(uint8 value, entropy_context *ctx)
{
    if (ctx->history[value] >= (2 * EVX_GB))
    {
        return evx_post_error(EVX_ERROR_INVALID_RESOURCE);
    }

    resolve_model_ctx(ctx);
    ...
    if (ctx->history[0] + ctx->history[1] >= 65536) {
        ctx->history[0] = (ctx->history[0] >> 1) | 1;
        ctx->history[1] = (ctx->history[1] >> 1) | 1;
    }

    return EVX_SUCCESS;
}

evx_status entropy_coder::decode_symbol(uint32 value, bit_stream *dest)
{
    if (EVX_PARAM_CHECK) 
    {
        if (!dest) 
...
evx_status entropy_coder::decode_symbol_ctx(uint32 value, bit_stream *dest, entropy_context *ctx)
{
    if (EVX_PARAM_CHECK)
    {
        if (!dest)
...
evx_status entropy_coder::flush_inverse_bits(uint8 value, bit_stream *dest) 
{
    if (EVX_PARAM_CHECK) 
    {
        if (!dest) 
...
evx_status entropy_coder::resolve_encode_scaling(bit_stream *dest) 
{
    if (EVX_PARAM_CHECK) 
    {
        if (!dest) 
...
evx_status entropy_coder::resolve_decode_scaling(uint32 *value, bit_stream *source, bit_stream *dest) 
{
    if (EVX_PARAM_CHECK) 
    {
    ...
    while (true) 
    {
        ...
        {
            // If our high value is less than half we do nothing 
            // (but prevent the loop from exiting).
        } 
...
evx_status entropy_coder::flush_encoder(bit_stream *dest) 
{
    if (EVX_PARAM_CHECK) 
    {
        if (!dest) 
...
evx_status entropy_coder::encode(bit_stream *source, bit_stream *dest, bool auto_finish) 
{
    if (EVX_PARAM_CHECK) 
    {
        if (!source || !dest) 
...
evx_status entropy_coder::decode(uint32 symbol_count, bit_stream *source, bit_stream *dest, bool auto_start) 
{
    if (EVX_PARAM_CHECK) 
    {
...
evx_status entropy_coder::encode_ctx(bit_stream *source, bit_stream *dest, entropy_context *ctx)
{
    if (EVX_PARAM_CHECK)
    {
        if (!source || !dest || !ctx)
    ...
    {
        if (EVX_SUCCESS != source->read_bit(&value) ||
            EVX_SUCCESS != encode_symbol_ctx(value, ctx) ||
            EVX_SUCCESS != resolve_encode_scaling(dest))
        {
            return evx_post_error(EVX_ERROR_INVALID_RESOURCE);
        }
...
evx_status entropy_coder::decode_ctx(uint32 symbol_count, bit_stream *source, bit_stream *dest, entropy_context *ctx)
{
    if (EVX_PARAM_CHECK)
    {
        if (0 == symbol_count || !source || !dest || !ctx)
        {
            return evx_post_error(EVX_ERROR_INVALIDARG);
        }
    }
    ...
    for (uint32 i = 0; i < symbol_count; ++i)
    {
        if (EVX_SUCCESS != decode_symbol_ctx(value, dest, ctx) ||
            EVX_SUCCESS != resolve_decode_scaling(&value, source, dest))
        {
            return evx_post_error(EVX_ERROR_EXECUTION_FAILURE);
        }
...
evx_status entropy_coder::start_decode(bit_stream *source) 
{
    uint8 bit = 0;
    value = 0;
...
evx_status entropy_coder::finish_encode(bit_stream *dest) 
{
    if (EVX_SUCCESS != flush_encoder(dest)) 
    {
        return evx_post_error(EVX_ERROR_EXECUTION_FAILURE);
