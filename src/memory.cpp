...
uint32 aligned_bit_copy(uint8 *source, uint32 source_bit_offset, uint32 copy_bit_count, uint8 *dest, uint32 dest_bit_offset) 
{
    if (EVX_PARAM_CHECK) 
    {
        if (!dest || !source ||
...
uint32 unaligned_bit_copy(uint8 *source, uint32 source_offset, uint32 copy_bit_count, uint8 *dest, uint32 dest_offset) 
{
    if (EVX_PARAM_CHECK) 
    {
